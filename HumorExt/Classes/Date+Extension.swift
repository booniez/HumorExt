...
public extension Date {
    ...
    func getNearestHour(_ interval: Int?) -> Date {
        var components = Calendar.current.dateComponents([.year, .month , .day , .hour , .minute], from: self)
        guard let min = components.minute else {
            return self
        }
        ...
        components.second = 0
        if min != 0 {
            components.hour? += (1 + (interval ?? 0))
        } else {
            components.hour? += (interval ?? 0)
        }
        return Calendar.current.date(from: components) ?? Date()
    }        
    
    func getPassedHour(_ interval: Int?) -> Date {
        var components = Calendar.current.dateComponents([.year, .month , .day , .hour , .minute], from: self)
        components.minute = 0
        components.second = 0
        components.hour? += (interval ?? 0)
    ...
    func getNextdDay(_ hour: Int?) -> Date {
        var components = Calendar.current.dateComponents([.year, .month , .day , .hour , .minute], from: self)
        components.minute = 0
        components.second = 0
    ...
    func getFixationDay(_ hour: Int?, day: Int?) -> Date {
        var components = Calendar.current.dateComponents([.year, .month , .day , .hour , .minute], from: self)
        components.minute = 0
        components.second = 0
        components.hour? = hour ?? 0
        components.day? += (day ?? 0)
        return Calendar.current.date(from: components) ?? Date()
    }
